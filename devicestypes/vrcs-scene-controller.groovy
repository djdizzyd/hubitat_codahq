/**
 *          THIS IS BROKEN FOR NOW!!!  DON'T USE IT.  USE THE OOB DRIVER
 * 
 *
 *  ZWave Scene Control
 *
...
def getdebug() { return true }

metadata {

  definition(name: "Leviton VRCS Scene Controller", namespace: "codahq-hubitat", author: "Ben Rimmasch") {
    capability "Actuator"
...
def buttonEvent(button) {
  if (debug) log.debug "Button ${button}"  
...
def zwaveEvent(hubitat.zwave.commands.sceneactivationv1.SceneActivationSet cmd) {
  ...
  if (debug) log.debug "SceneActivationSet cmd ${cmd}"  
...
def zwaveEvent(hubitat.zwave.commands.associationv2.AssociationGroupingsReport cmd) {
  if (debug) log.debug "AssociationGroupingsReport cmd ${cmd}"  
...
def zwaveEvent(hubitat.zwave.Command cmd) {
  ...
  if (debug) log.debug "Unhandled commands cmd ${cmd}"  
...
def configurationCmds() {
  if (debug) log.debug "configurationCmds"  
  ...
  if (debug) log.debug "configurationCmds commands before associate '${commands}'"

  commands << associateHub()
  
  if (debug) log.debug "configurationCmds commands after associate '${commands}'"
  ...
  if (debug) log.debug "configurationCmds commands final '${commands}'"
...
def associateHub() {
  ...
  for (def buttonNum = 1; buttonNum <= integer(getDataByName("numberOfButtons")); buttonNum++) {
    ...
    def tmp = zwave.sceneControllerConfV1.sceneControllerConfSet(groupId: buttonNum, sceneId: buttonNum, dimmingDuration: 0).format()
    if (debug) log.debug "Huh? Why does this return a null? ${tmp}"
    tmp.properties.each { log.info it }
    commands << tmp
    ...
    if (buttonNum == 1) {
      ...
      if (getDataByName("associatedLoad") != "0" && getDataByName("associatedLoad") != null) {
        ...
        if (debug) log.debug "Associate Load"
        ...
        if (debug) log.debug "Associate Button 1"
        ...
        if (getDataByName("associatedLoadId")) {
          if (debug) log.debug "Disassociate load"
    ...
    // For the other buttons:
      ...
      def cmd = zwave.associationV1.associationSet(groupingIdentifier: buttonNum, nodeId: zwaveHubNodeId).format()
      if (debug) log.debug "Button ${buttonNum} of 2-${getDataByName("numberOfButtons")} to ${zwaveHubNodeId} with ${cmd}"
      commands << cmd
    }
  }
  
  if (debug) log.debug "Leaving associateHub with ${commands}"
